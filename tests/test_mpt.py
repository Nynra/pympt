...
from src.mpt.exceptions import (
    InvalidNodeError,
    LeafPathError,
    ExtensionPathError,
    BranchPathError,
...
class Test_proof_of_inclusion(unittest.TestCase):
    ...
    files = {
        "lots_of_proofs": "tests/test_proofs/mpt_lots_of_poi.pkl",
    ...
    def test_proof_many(self):
        ...
        for i in range(len(proofs)):
            for j in range(len(expected[i])):
                self.assertEqual(
                    proofs[i][j], expected[i][j], "Proof does not match expected"
                )
    ...
    def test_valid(self):
        ...
        for i in range(len(proofs)):
            self.assertTrue(
                trie.verify_proof_of_inclusion(data[i][0], proofs[i]),
                "Proof does not validate for {}, got {}.".format(data[i], proofs[i]),
            )
    ...
    def test_verify_one_point_removed(self):
        ...
        self.assertFalse(
            trie.verify_proof_of_inclusion(data[2][0], proof),
...
class Test_proof_of_exclusion(unittest.TestCase):
    ...
    files = {
        "lots_of_proofs": "tests/test_proofs/mpt_lots_of_poe.pkl",
    ...
    def test_proof_many(self):
        ...
        for i in range(len(proofs)):
            self.assertEqual(
                proofs[i],
                expected[i],
                "Proof does not match expected for {}, expected {}, got {}.".format(
    ...
    def test_valid(self):
        ...
        for cnt, p in enumerate(proofs):
            self.assertTrue(
                trie.verify_proof_of_exclusion(keys[cnt][0], p), "Proof is not valid."
            )
    ...
    def test_verify_one_point_removed(self):
        ...
        self.assertFalse(
            trie.verify_proof_of_exclusion(b"wolf", proof), "Proof should not be valid."
        )
    ...
    def test_verify_one_point_added(self):
        ...
        self.assertEqual(
            trie.verify_proof_of_exclusion(b"wolf", proof),
            False,
            "Proof should not be valid.",
        )
